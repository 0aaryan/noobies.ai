        ...
        with st.form(key="generate_video_form"):
            ...
            if submit_button:
                with st.spinner("Generating Script"):
                    ...
                    transcript, message = transcript_downloader.download_text(url)
                    print(transcript)
                    if transcript is None or transcript == "":
                        st.error("This video does not have a transcript")
                        st.info("We are working on a solution for this")
                        return
                    else:
                        st.success(message)
                        topic = transcript
...
if __name__ == "__main__":
    init()
    get_clarifai_pat()
    if os.environ.get("CLARIFAI_PAT") is not None:
        main()
    else:
        st.error("Please add your Clarifai PAT")
